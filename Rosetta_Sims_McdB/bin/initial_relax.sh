...
#
#
#SBATCH --job-name=carboxysome_pp_ir
#SBATCH --account=ave0
...
ATPASE_ID='McdA'
DATE='01_09_2022'
SCRATCH_DIR=/scratch/ave_root/ave0/limcaoco/relax_${ATPASE_ID}_${DATE}
...
${PATH_TO_ROSETTA}/main/source/bin/rosetta_scripts.default.linuxgccrelease \
    ...
    @ ${PATH_TO_PROJ}/input_files/options/initial_relax.options \
    -in:file:s ${PATH_TO_PROJ}/input_files/receptors/${ATPASE_ID}_elongatus.pdb\
    -run:jran ${JOB_ID} \
    -out:suffix _${TASK_ID}_${JOB_ID} \
    -out:nstruct 20 \
    -out:file:silent ${SCRATCH_DIR}/out_files/${ATPASE_ID}_relax_${JOB_ID}_${TASK_ID}.out \
    > ${SCRATCH_DIR}/${ATPASE_ID}_relax_${JOB_ID}_${TASK_ID}.log
##
